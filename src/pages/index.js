...
export default () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://docs.radxa.com/json/baidu_tongji_data.json")
      .then((res) => {
      ...
      .catch((err) => {});
    axios
      .get("https://docs.radxa.com/json/commit_update_data.json")
      .then((res) => {
        const filteredData = res.data.items.filter((item) => {
          return (
            item.title.indexOf("Merge") !== 0 &&
            !specialPathRegex.test(item.page_url)
          );
        });
  ...
  return (
    <Layout>
      ...
      <section
        className={styles["doc-series-container"]}
        ...
        <div>
          <div className={styles["pr-list"]}>
            <div className={styles["pr-category"]}>
              <CenteredTabs
                datalist={Object.keys(home_data)}
              ...
              <div className={styles.line_info} ref={infoRef}>
                ...
                <div className={styles.doc_links}>
                  {home_data[indexCategory].length > 0 &&
                    home_data[indexCategory][seriesKey]?.products[
                      productKey
                    ]?.docs.map((item, index) => {
                      return (
          ...
          <div className={styles["right-list"]}>
            <div
              style={{ display: `${hotTopics.length > 0 ? "block" : "none"}` }}
            >
                      ...
                      return (
                        <li key={index}>
                          {current.breadcrumbs.length > 0 &&
                            current.breadcrumbs.map((_item, _index) => {
                              if (_index > 0)
                                return (
                                  <>
                                    <Link
                                    ...
                                    {_index !==
                                      current.breadcrumbs.length - 1 && svgEle}
                                  </>
                                );
                            })}
                        </li>
            ...
            <div
              style={{ display: `${hotUpdates.length > 0 ? "block" : "none"}` }}
            >
              <p>{currentLocale ? "更新" : "Update"}</p>
              <ol>
                {hotUpdates.length > 0 &&
                  hotUpdates.map((item, index) => {
                    if (index < 5)
                      return (
            ...
            <div>
              ...
              <ol>
                {official_selection.length > 0 &&
                  official_selection.map((item, index) => {
                    return (
                      <li key={index}>
