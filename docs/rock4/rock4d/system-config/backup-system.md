...
在线系统备份是指在backup system.md上备份正在运行的系统。
...
在backup system.md上使用 `curl` 命令下载脚本。
...
离线系统备份是指将backup system.md的系统以可移动介质的方式连接到 PC 上备份。
...
将backup system.md的系统启动盘转为可移动介质后，连接到 PC 上备份（推荐在 Ubuntu 系统下备份）。
...
终端输出类似信息：其中 `sda` 为演示的backup system.md系统盘，您可以通过安全插拔可移动介质来判断对应设备信息。
