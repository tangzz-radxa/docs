...
串口调试是嵌入式开发中通过串行通信接口 (UART) 与uart debug.md交互的核心手段，通过串口工具可以查看系统输出的信息和进行调试。

## 硬件准备

- uart debug.md： 瑞莎 ROCK 4D
- USB 串口数据线： 将uart debug.md的 UART0 接口连接到 PC 机的 USB 端口
- 电源适配器： 给uart debug.md供电（支持 PD 协议，5V 电源输入）
